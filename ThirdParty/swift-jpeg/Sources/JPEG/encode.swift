...
extension JPEG.Data.Planar.Plane {
    fileprivate func load(x: Int, y: Int, limit: SIMD8<Float>)
...
extension JPEG.Data.Spectral.Plane {
    private static var zigzag: Block8x8<Int> {
        @inline(__always) func row(_ h: Int) -> SIMD8<Int> {
            .init(
    ...
    private static func fdct8(_ g: Block8x8<Float>, shift: Float) -> Block8x8<Float> {
        // even rows
    ...
    private static func fdct8x8(_ g: Block8x8<Float>, shift: Float) -> Block8x8<Float> {
    ...
    mutating func fdct(
        _ plane: JPEG.Data.Planar<Format>.Plane,
...
extension JPEG.Data.Planar {
    ...
    public func fdct(quanta: [JPEG.Table.Quantization.Key: [UInt16]])
    -> JPEG.Data.Spectral<Format> {
        ...
        for (p, plane): (Int, JPEG.Data.Planar<Format>.Plane) in
            ...
            spectral[p].fdct(
                plane, quanta: spectral.quanta[spectral[p].q],
                precision: precision
            )
        }
...
extension JPEG.Data.Spectral {
    ...
    public func encode() -> JPEG.Header.Frame {
        do {
            ...
            preconditionFailure((error as? any JPEG.Error)?.message ?? "\(error)")
        }
    }
}
extension JPEG.Layout {
    ...
    /// array.
    public var scans: [JPEG.Header.Scan] {
...
extension JPEG.Table.Huffman.Subtree {
    private var children: [JPEG.Table.Huffman<Symbol>.Subtree<Element>] {
        switch self.node {
        case .leaf:
            return []
...
extension JPEG.Table.Huffman {
    ...
    private static func limit(height: Int, of uncompacted: ArraySlice<Int>) -> [Int] {
    ...
    private static func assign(_ symbols: Int, levels: [Int]) -> [Encoder.Codeword] {
    ...
    public init(frequencies: [Int], target: Selector) {
        precondition(
            frequencies.count == 256,
            "frequency array must have exactly 256 elements"
        )
        precondition(
            !frequencies.allSatisfy{ $0 <= 0 },
        ...
        var heap: General.Heap<Int, Subtree<Void>> = .init(
            sorted.reversed().map {
        ...
        while let first: (key: Int, value: Subtree<Void>) = heap.dequeue() {
            guard let second: (key: Int, value: Subtree<Void>) = heap.dequeue() else {
...
extension JPEG.Table.Huffman {
    @_spi(_Testable) public struct Encoder {
        @_spi(_Testable) public struct Codeword {
            // the inhabited bits are in the most significant end of the `UInt16`
            @_spi(_Testable) public let bits: UInt16

            @General.Storage<UInt16>
            @_spi(_Testable) public var length: Int
        }

        private let storage: [Codeword]
...
extension JPEG.Table.Huffman {
    @_spi(_Testable) public func encoder() -> Encoder {
...
extension JPEG.Table.Huffman.Encoder {
    @_spi(_Testable) public subscript(symbol: Symbol) -> Codeword {
...
extension JPEG.Bitstream {
    mutating func append(composite: Composite.DC, table: JPEG.Table.HuffmanDC.Encoder) {
    ...
    mutating func append(composite: Composite.AC, table: JPEG.Table.HuffmanAC.Encoder) {
...
extension JPEG.Bitstream.AnySymbol {
    @_spi(_Testable) public static func frequencies<S>(
        of path: KeyPath<S.Element, Self>,
        in sequence: S
    ) -> [Int]
        where S: Sequence {
    ...
    -> ([UInt8], JPEG.Table.HuffmanDC, JPEG.Table.HuffmanAC) {
        ...
        .init(unsafeUninitializedCapacity: count) {
            ...
            for (x, y): (Int, Int) in self.indices {
                ...
                ($0.baseAddress! + y * self.units.x + x).initialize(
                    to: self.encode(x: x, y: y, predecessor: &predecessor)
                )
            }
        ...
        let frequencies: (dc: [Int], ac: [Int]) = (
            JPEG.Bitstream.Symbol.DC.frequencies(
                of: \.0.decomposed.symbol,
                in: composites
            ),
            JPEG.Bitstream.Symbol.AC.frequencies(
                of: \.decomposed.symbol,
...
extension JPEG.Data.Spectral {
    // sequential mode
    private func encode(components: [(c: Int, component: JPEG.Scan.Component)])
    -> ([UInt8], [JPEG.Table.HuffmanDC], [JPEG.Table.HuffmanAC]) {
        ...
        .init(unsafeUninitializedCapacity: count) {
            ...
            for ((p, component), factor): ((Int, JPEG.Scan.Component), (x: Int, y: Int)) in
                ...
                for (mx, my): (x: Int, y: Int) in (0, 0) ..< self.blocks {
                    ...
                    for (i, (x, y)): (offset: Int, element: (x: Int, y: Int)) in (
                            start ..< end
                        ).enumerated() {
                        let composite: JPEG.Bitstream.Composite.DC,
        ...
        .init(
            uniqueKeysWithValues: tables.dc.map {
                ($0.target, $0.encoder())
            }
        )
        let ac: [JPEG.Table.HuffmanAC.Selector: JPEG.Table.HuffmanAC.Encoder] =
        .init(
            uniqueKeysWithValues: tables.ac.map {
                ($0.target, $0.encoder())
            }
        )
        var offset: Int                  = 0
        ...
        for base: Int in
            ...
            for descriptor: Descriptor in descriptors {
                ...
                for (composite, ac): (
                        JPEG.Bitstream.Composite.DC,
                        [JPEG.Bitstream.Composite.AC]
                    ) in
    ...
    private func encode(bit a: Int, components: [(c: Int, component: JPEG.Scan.Component)])
    ->  [UInt8] {
        guard components.count > 1 else {
            // noninterleaved
            precondition(components.count == 1, "components array cannot be empty")
    ...
    -> (
        ...
        .init(
            band: scan.band, bits: scan.bits, components: scan.components.map(\.component)
        )
        ...
        case (initial: true,  band: let band):
            precondition(
                ...
                "progressive ac scan cannot be interleaved"
            )
...
extension JPEG.AnyTable {
    static func serialize(selector: Self.Selector) -> UInt8 {
        switch selector {
        case \.0:
            return 0
        case \.1:
...
extension JPEG.Table {
    ...
    public static func serialize(_ dc: [HuffmanDC], _ ac: [HuffmanAC]) -> [UInt8] {
        var bytes: [UInt8] = []
    ...
    ///     A marker segment body. This array does not include the marker type
    ///     indicator, or the marker segment length field.
    public static func serialize(_ tables: [Quantization]) -> [UInt8] {
        var bytes: [UInt8] = []
...
extension JPEG.Bytestream.Destination {
    ...
    public mutating func format(marker: JPEG.Marker) throws {
        guard let _: Void    = self.write([0xff, marker.code]) else {
    ...
    ///     indicator, or the marker segment length field.
    public mutating func format(marker: JPEG.Marker, tail: [UInt8]) throws {
        let length: Int      = tail.count + 2
        let bytes: [UInt8]   =
        [0xff, marker.code] + [UInt8].store(length, asBigEndian: UInt16.self) + tail
        guard let _: Void    = self.write(bytes) else {
            throw JPEG.FormattingError.invalidDestination
        }
    }
    ...
    public mutating func format(prefix: [UInt8]) throws {
        guard let _: Void = self.write(prefix) else {
            throw JPEG.FormattingError.invalidDestination
        }
    }
}
...
extension JPEG.Data.Spectral {
    ...
    /// -   Parameter stream:
    ///     A destination bytestream.
    public func compress<Destination>(stream: inout Destination) throws
